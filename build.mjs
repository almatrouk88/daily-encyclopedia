...
const idx={ note:"فهرس مقالات الموسوعة", categories:[...new Set(entries.map(e=>e.category))],
  articles:entries.map(e=>({id:e.id,title:e.title,category:e.category,reading_min:e.reading_min||5,day:e.day,file:e.file,
    text:[e.main_idea,(e.body||[]).join(" "),(e.summary||[]).join(" "),(e.reflection||[]).join(" ")].join(" ")})) };
...
var READ=readMap();
var LAST=null; try{LAST=JSON.parse(localStorage.getItem('enc-last')||'null');}catch(e){}
(function(){
...
Promise.all([fetch('index.json',{cache:'no-store'}).then(function(r){return r.json();}),fetch('manifest.json',{cache:'no-store'}).then(function(r){return r.json();})]).then(function(res){
 ...
 // إذا أنهيتَ آخر مقالة كنتَ فيها (أو لا يوجد موضع محفوظ)، اجعل «متابعة القراءة» تأخذك للتالي غير المقروء تلقائيًّا
 (function(){
   var box=document.getElementById('resume');
   var lastArt=LAST&&arts.filter(function(a){return a.file===LAST.file;})[0];
   if(!LAST || (lastArt&&READ[lastArt.id])){
     if(_nu){ box.innerHTML='<a class="resume-btn" href="'+_nu.file+'"><span class="rt">▶ التالي غير المقروء</span><span class="rs">'+_nu.title+'</span></a>'; }
     else if(arts.length){ box.innerHTML='<div class="resume-btn"><span class="rt">✓ أنهيتَ كلّ المقالات</span><span class="rs">أحسنت!</span></div>'; }
   }
 })();
...
function run(){var q=norm(document.getElementById('q').value);
 var res=DATA.filter(function(x){return !q||norm(x.title).indexOf(q)>=0||norm(x.category).indexOf(q)>=0||norm(x.text||'').indexOf(q)>=0;});
